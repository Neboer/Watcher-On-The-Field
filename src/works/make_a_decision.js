const {maturation, cut_good_wheat, plant_seed_on, craft_bread, eat_bread, make_hey, pile_bone_meal, pick_up, move_to_center, badly_make_hey, badly_pile_seed} = require('../actions')
...
function analyse_a_section_and_make_decision(game_section) {
    ...
    if (s.empty_slots_count <= 1) {// 物品栏空间不足，优先堆积。
        if (s.nearest_composter && s.seed_count > 64) return [badly_pile_seed, s.nearest_composter]
        if (s.wheat_count >= 9 && s.nearest_crafting_table) return [badly_make_hey, s.nearest_crafting_table]
    }
